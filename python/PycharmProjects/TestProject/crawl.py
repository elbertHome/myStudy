...
myProxies = {'http': 'http://15.105.253.250:8080',
             'https': 'https://15.105.253.250:8080'}
...
class Retriever(object):
    ...
    def get_file(self, url, default="index.html"):
        """Creeate usable local filename from URL"""
    ...
    def download(self):
        ...
        try:
            opener = urllib.FancyURLopener(proxies=myProxies)
            retval = opener.retrieve(self.url, self.file)
...
class Crawler(object):
    ...
    def get_page(self, url, media=False):
        ...
        if ftype not in (".html", ".htm", "shtml"):
            return

        for link in r.parse_links():
            # discard the mail address
            if link.startswith("mailto:"):
                print "... discarded, mailto link"
                continue

            # discard the media file if media == False
            if not media:
